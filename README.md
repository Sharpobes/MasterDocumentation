...
- разделы, папки, подпапки и документы с drag-and-drop, уникальными названиями, избранным, недавними файлами и корзиной; щелчок по пустому месту списка снимает выбор папки, а перетаскивание мимо строк или команда «Переместить на уровень выше» выносит элемент из папки;
- несколько вкладок, восстановление сессии, автосохранение и аварийные черновики; настройки открываются такой же вкладкой в общем ряду и имеют кнопки «Применить», «Сохранить» и «Отмена»;
- TipTap/ProseMirror JSON как основной формат документа, HTML и plain text для экспорта и поиска;
- заголовки H1–H6, живая структура и оглавление с переходом к разделу: раскрытые ветки не схлопываются при наборе, а курсор в тексте подсвечивает и прокручивает нужный заголовок;
- шрифты, размеры, цвета, интервалы, отступы, выравнивание, регистр, формат по образцу и LTR/RTL; параметры абзаца задаются в единицах измерения из настроек;
- списки, чек-листы, цитаты, спойлеры, ссылки с показом адреса при наведении, таблицы с фиксированной шириной столбцов и переносом строки по `Shift+Enter`, блоки кода с локальной подсветкой;
...
- вложения с SHA-256-дедупликацией: одинаковый файл не сохраняется на диске повторно, двойной щелчок ведёт к картинке в тексте, предпросмотр открывается отдельной кнопкой;
...
- импорт TXT, Markdown, HTML, RTF, DOCX и PDF: из PDF переносится текст, сам файл сохраняется вложением;
...
Подпись кода включается ключами `-CertificatePath`/`-CertificateThumbprint` или параметрами Trusted Signing; `-RequireSignature` не даёт собрать неподписанный выпуск. Предупреждение SmartScreen снимает не сам факт подписи, а накопленная репутация скачиваний — мгновенного обхода нет. Подробности — в [docs/CODE_SIGNING.md](docs/CODE_SIGNING.md).
