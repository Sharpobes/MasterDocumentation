...
{
    public string ItemTitle=>TitleBox.Text.Trim();public bool IsFolder=>KindBox.SelectedIndex==1;public long? ParentId=>(ParentBox.SelectedItem as ParentChoice)?.Id;public long? TemplateId=>(TemplateBox.SelectedItem as TemplateChoice)?.Id;
    public NewItemDialog(IEnumerable<NodeItem> roots,IEnumerable<NodeItem> templates,bool folder,long? selectedParent)
    {InitializeComponent();var parents=new List<ParentChoice>{new(null,"Корень библиотеки")};AddFolders(roots,parents,"");ParentBox.ItemsSource=parents;ParentBox.SelectedItem=parents.FirstOrDefault(x=>x.Id==selectedParent)??parents[0];TemplateBox.ItemsSource=new[]{new TemplateChoice(null,"Без шаблона")}.Concat(templates.Select(x=>new TemplateChoice(x.Id,x.Title))).ToList();TemplateBox.SelectedIndex=0;KindBox.SelectedIndex=folder?1:0;Loaded+=(_,_)=>TitleBox.Focus();}
    ...
    private void Kind_Changed(object sender,SelectionChangedEventArgs e){if(TemplateBox is not null)TemplateBox.IsEnabled=KindBox.SelectedIndex==0;}
    private void Template_Changed(object sender,SelectionChangedEventArgs e){if(TemplateBox.SelectedIndex>0)KindBox.SelectedIndex=0;}
