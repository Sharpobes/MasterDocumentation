...
public sealed class SqliteDocumentStore : IDocumentStore
{
    ...
    public List<NodeItem> LoadTree(string? query = null)
    {
        ...
        command.CommandText = string.IsNullOrWhiteSpace(query)
            ? $"SELECT Id,ParentId,IsFolder,Title,SortOrder,CreatedAt,ModifiedAt,IsTemplate FROM Nodes WHERE DeletedAt IS NULL AND {AccessFilter} ORDER BY SortOrder,Title"
            : $"WITH RECURSIVE matched(Id) AS (SELECT n.Id FROM Nodes n WHERE n.DeletedAt IS NULL AND {AccessFilterAliased} AND (n.Title LIKE $q OR (n.IsFolder=0 AND n.Id IN (SELECT CAST(NodeId AS INTEGER) FROM SearchIndex WHERE SearchIndex MATCH $fts)))),visible(Id) AS (SELECT Id FROM matched UNION SELECT n.ParentId FROM Nodes n JOIN visible v ON n.Id=v.Id WHERE n.ParentId IS NOT NULL) SELECT DISTINCT n.Id,n.ParentId,n.IsFolder,n.Title,n.SortOrder,n.CreatedAt,n.ModifiedAt,n.IsTemplate FROM Nodes n JOIN visible v ON v.Id=n.Id WHERE n.DeletedAt IS NULL AND {AccessFilterAliased} ORDER BY n.SortOrder,n.Title";
        AddUser(command);
    ...
    {
        title=title.Trim();if(title.Length==0)throw new ArgumentException("Название не может быть пустым.");if(TitleConflict(parentId,title,folder) is string conflict)throw new InvalidOperationException(conflict);
    ...
    public bool TitleExists(long? parentId,string title,long? exceptId=null,bool? isFolder=null){using var c=Open();using var cmd=c.CreateCommand();cmd.CommandText="SELECT EXISTS(SELECT 1 FROM Nodes WHERE ParentId IS $p AND Title=$t COLLATE NOCASE AND DeletedAt IS NULL AND ($e IS NULL OR Id<>$e) AND ($kind IS NULL OR IsFolder=$kind))";cmd.Parameters.AddWithValue("$p",parentId is null?DBNull.Value:parentId);cmd.Parameters.AddWithValue("$t",title.Trim());cmd.Parameters.AddWithValue("$e",exceptId is null?DBNull.Value:exceptId);cmd.Parameters.AddWithValue("$kind",isFolder is null?DBNull.Value:isFolder);return Convert.ToInt64(cmd.ExecuteScalar())!=0;}
    /// <summary>
    /// Описание занятого названия или null, если название свободно. Сравнение идёт внутри своего
    /// типа: папка и документ названия друг у друга не занимают. Учитываются и невидимые
    /// приватные страницы других пользователей; удалённое в корзину названий не занимает.
    /// </summary>
    private string? TitleConflict(long? parentId,string title,bool isFolder,long? exceptId=null,string where="В этой папке")
    {
        ...
        cmd.CommandText="SELECT Title,IsFolder,IsPrivate,Owner FROM Nodes WHERE ParentId IS $p AND Title=$t COLLATE NOCASE AND IsFolder=$kind AND DeletedAt IS NULL AND ($e IS NULL OR Id<>$e) LIMIT 1";
        cmd.Parameters.AddWithValue("$p",parentId is null?DBNull.Value:parentId);
        cmd.Parameters.AddWithValue("$t",title.Trim());
        cmd.Parameters.AddWithValue("$kind",isFolder);
        cmd.Parameters.AddWithValue("$e",exceptId is null?DBNull.Value:exceptId);
        using var r=cmd.ExecuteReader();
        return r.Read()?TitleConflicts.Describe(r.GetString(0),r.GetBoolean(1),r.GetBoolean(2),r.GetString(3),where):null;
    }
    public void Rename(long id, string title) { var node=FindNode(id)??throw new InvalidOperationException("Элемент не найден.");if(TitleConflict(node.ParentId,title,node.IsFolder,id) is string conflict)throw new InvalidOperationException(conflict);Execute("UPDATE Nodes SET Title=$v,ModifiedAt=$m WHERE Id=$id", id, title.Trim()); if(node.IsFolder)RefreshBranchSearchIndex(id);else RefreshSearchIndex(id); }
    ...
    public void SetDocumentGuid(long id,string guid)
    {
        ...
        using var c=Open();using var cmd=c.CreateCommand();
        cmd.CommandText="UPDATE Nodes SET Guid=$g WHERE Id=$id AND IsFolder=0 AND NOT EXISTS(SELECT 1 FROM Nodes o WHERE o.Guid=$g AND o.Id<>$id)";
        cmd.Parameters.AddWithValue("$id",id);cmd.Parameters.AddWithValue("$g",guid);
        cmd.ExecuteNonQuery();
    }
    ...
    public void Restore(long id) { using var c=Open();long? parent;string original;bool isFolder;using(var find=c.CreateCommand()){find.CommandText="SELECT ParentId,Title,IsFolder FROM Nodes WHERE Id=$id AND DeletedAt IS NOT NULL";find.Parameters.AddWithValue("$id",id);using var reader=find.ExecuteReader();if(!reader.Read())throw new InvalidOperationException("Элемент не найден в корзине.");parent=reader.IsDBNull(0)?null:reader.GetInt64(0);original=reader.GetString(1);isFolder=reader.GetBoolean(2);}var title=original;for(var i=2;;i++){using var conflict=c.CreateCommand();conflict.CommandText="SELECT EXISTS(SELECT 1 FROM Nodes WHERE ParentId IS $p AND Title=$t COLLATE NOCASE AND IsFolder=$kind AND DeletedAt IS NULL AND Id<>$id)";conflict.Parameters.AddWithValue("$p",parent is null?DBNull.Value:parent);conflict.Parameters.AddWithValue("$t",title);conflict.Parameters.AddWithValue("$kind",isFolder);conflict.Parameters.AddWithValue("$id",id);if(Convert.ToInt64(conflict.ExecuteScalar())==0)break;title=$"{original} (восстановлено {i})";}using var cmd=c.CreateCommand();cmd.CommandText="WITH RECURSIVE branch(Id) AS (SELECT $id UNION ALL SELECT n.Id FROM Nodes n JOIN branch b ON n.ParentId=b.Id) UPDATE Nodes SET DeletedAt=NULL,ModifiedAt=$d,Title=CASE WHEN Id=$id THEN $title ELSE Title END WHERE Id IN branch";cmd.Parameters.AddWithValue("$id",id);cmd.Parameters.AddWithValue("$title",title);cmd.Parameters.AddWithValue("$d",DateTime.UtcNow.ToString("O"));cmd.ExecuteNonQuery();RefreshSearchIndex(id); }
    ...
    /// <summary>
    /// Чтение старой таблицы Settings локальной базы. Не входит в <see cref="IDocumentStore"/>:
    /// состояние интерфейса больше не хранится в базе документации, метод нужен только для
    /// разового переноса ранее сохранённых значений в <see cref="LocalStateService"/>.
    /// </summary>
    ...
    public (string StoredName,bool IsUnused) RemoveAttachment(long attachmentId,bool ignoreVersions=false){string stored;long documentId;using(var c=Open()){using var tx=c.BeginTransaction();using(var find=c.CreateCommand()){find.Transaction=tx;find.CommandText="SELECT StoredName,DocumentId FROM Attachments WHERE Id=$id";find.Parameters.AddWithValue("$id",attachmentId);using var reader=find.ExecuteReader();if(!reader.Read())throw new InvalidOperationException("Вложение не найдено.");stored=reader.GetString(0);documentId=reader.GetInt64(1);}using(var remove=c.CreateCommand()){remove.Transaction=tx;remove.CommandText="DELETE FROM Attachments WHERE Id=$id";remove.Parameters.AddWithValue("$id",attachmentId);remove.ExecuteNonQuery();}tx.Commit();}RefreshSearchIndex(documentId);if(ignoreVersions)PurgeAssetFromVersions(stored);using var check=Open();using var cmd=check.CreateCommand();cmd.CommandText="SELECT (SELECT COUNT(*) FROM Attachments WHERE StoredName=$name)+(SELECT COUNT(*) FROM Nodes WHERE EditorJson LIKE $like OR Html LIKE $like)"+(ignoreVersions?"":"+(SELECT COUNT(*) FROM DocumentVersions WHERE EditorJson LIKE $like OR Html LIKE $like)");cmd.Parameters.AddWithValue("$name",stored);cmd.Parameters.AddWithValue("$like","%"+stored+"%");return(stored,Convert.ToInt64(cmd.ExecuteScalar())==0);}
    /// <summary>
    /// Убирает снимки истории, ссылающиеся на удалённое вложение: иначе автосохранение,
    /// сделанное секунду назад, навсегда удерживает файл на диске, а восстановление такой версии
    /// вернуло бы в текст ссылку на несуществующий файл. Закреплённые версии не трогаются.
    /// </summary>
    private void PurgeAssetFromVersions(string storedName){using var c=Open();using var cmd=c.CreateCommand();cmd.CommandText="DELETE FROM DocumentVersions WHERE IsPinned=0 AND (EditorJson LIKE $like OR Html LIKE $like)";cmd.Parameters.AddWithValue("$like","%"+storedName+"%");cmd.ExecuteNonQuery();}
