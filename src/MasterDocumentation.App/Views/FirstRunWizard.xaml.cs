...
public partial class FirstRunWizard : Window
{
    ...
    {
        InitializeComponent();WindowPlacementService.PrepareOwnerlessWindow(this);DataPathBox.Text=AppPaths.Data;
        LanguageBox.SelectedIndex=LocalizationService.IsEnglish?1:0;
        LanguageBox.SelectionChanged+=Language_Changed;
        LocalizationService.Apply(this);
    }
    private void Language_Changed(object sender,System.Windows.Controls.SelectionChangedEventArgs e)
    {
        if(LanguageBox.SelectedItem is not System.Windows.Controls.ComboBoxItem item)return;
        LocalizationService.SetLanguage(LocalizationService.SourceContent(item));
        LocalizationService.Apply(this);
    }
    private void Browse_Click(object sender,RoutedEventArgs e){var dialog=new OpenFolderDialog{Title=LocalizationService.T("Выберите папку для хранилища MasterDocumentation")};if(dialog.ShowDialog(this)==true)DataPathBox.Text=Path.Combine(dialog.FolderName,"MasterDocumentationData");}
    ...
    private void Skip_Click(object sender,RoutedEventArgs e){SelectedLanguage=LocalizationService.CurrentLanguage;SelectedTheme="Тёмная";SelectedDataPath=AppPaths.Data;AutomaticBackups=true;FirstDocumentTitle=null;DialogResult=true;}
    private void Finish_Click(object sender,RoutedEventArgs e)
    {
        ...
        SelectedLanguage=LanguageBox.SelectedItem is System.Windows.Controls.ComboBoxItem language?LocalizationService.SourceContent(language):LocalizationService.Russian;SelectedTheme=ThemeBox.SelectedItem is System.Windows.Controls.ComboBoxItem theme?LocalizationService.SourceContent(theme):"Тёмная";SelectedDataPath=Path.GetFullPath(path);AutomaticBackups=AutomaticBackupsCheck.IsChecked==true;FirstDocumentTitle=CreateDocumentCheck.IsChecked==true?FirstDocumentBox.Text.Trim():null;DialogResult=true;
    }
}
